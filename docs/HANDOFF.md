...
## Numbers at handoff (2026-09-23, pass BG)
...
Stories: from grade 3 up, 229 of 319 module stories are still three terse lines rather than the four-step Gladwell arc (docs/THIN-STORIES.md); grades 3 and 4 and grade 5 math and science are done and are the models, and each pass rewrites a batch (grade 5 reading, writing and history next, then grade 6). Maps: nine map games (continents, oceans, hemispheres, Texas regions, US regions, twenty-five states, Europe, Asia, Africa, South America) keep their regions as invisible hit areas and wait for Mikey's Leonardo paintings M1 to M7 (docs/ART-REQUESTS.md); real students see a map only once its painting is in place. When a painting arrives, run tools/map-overlay.mjs and trace the hit areas to it. Games: Mikey wants visual, hands-on kinds, never more decks; maps get Leonardo paintings by M serial behind the tap regions, and Europe and the US states are the next maps.
