...
const rows = new Set([...ledger.matchAll(/^\| ((?:CS|[SLD])\d+) \|/gm)].map((m) => m[1]));
...
const drawingSerials = Array.from({ length: 27 }, (_, i) => `D${i + 1}`);
const courseSerials = Object.values(COURSE_STORIES).map((s) => s.art);
const missing = [...storySerials, ...letterSerials, ...drawingSerials, ...courseSerials].filter((x) => !rows.has(x));
...
const used = new Set([...storySerials, ...letterSerials, ...drawingSerials, ...courseSerials]);
